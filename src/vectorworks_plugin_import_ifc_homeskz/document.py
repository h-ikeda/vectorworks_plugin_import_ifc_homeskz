        ...
        "walls": [
            {
                ...
                # 配筋情報(立上りの ``配筋`` レコードに設定する)。IFC の
                # Pset_Reinforcement から取り、鉄筋オブジェクトは描かずデータタグ等で
                # 参照できるよう壁オブジェクトのレコードに入れる。top=上端主筋・
                # bottom=下端主筋・vertical=縦筋(いずれも呼び名文字列)。
                "reinforcement": {
        ...
        "slabs": [
            {
                ...
                # 配筋情報(底盤の ``配筋`` レコードに設定する)。IFC の
                # Pset_Reinforcement は短辺方向・長辺方向の 2 方向配筋なので、底盤の
                # 平面バウンディングボックス(短辺=extent の小さい軸)からグローバル
                # X・Y 方向へ置き換えた仕様(呼び名文字列)。鉄筋オブジェクトは描かず
                # スラブオブジェクトのレコードに入れてデータタグ等で参照できるようにする。
        ...
        "sheets": [
            {
                ...
                "viewport": {
                    ...
                    # このビューポートで非表示にするクラス名(省略可・既定は無し)。
                    # 表示レイヤに乗っていても、ここに挙げたクラスの図形だけは非表示に
                    # する。現状はどの伏図も指定しない(省略)。
                    "hidden_classes": []
                }
            }
...
配筋(立上り=上端主筋・下端主筋・縦筋、底盤=X 方向・Y 方向)は独立した命令ではなく、
walls / slabs の各命令の ``reinforcement`` フィールドとして持つ(鉄筋オブジェクトは
描かず、描画フェーズが壁・スラブオブジェクトの ``配筋`` レコードに設定する)。
"""
...
class WallReinforcementCommand(TypedDict):
    """立上り(基礎梁)の配筋。壁オブジェクトの ``配筋`` レコードに設定する。

    top=上端主筋・bottom=下端主筋・vertical=縦筋(いずれも鋼種を除いた呼び名文字列。
    縦筋は脚数を ``1-`` に固定した ``1-D10@300`` 形式)。IFC の Pset_Reinforcement から
    取得し、無ければ既定値(上下 1-D13・縦筋 1-D10@250)を使う。鉄筋オブジェクトは
    描かず、データタグ等で参照できるよう壁オブジェクトのレコードに入れる。
    """
...
class SlabReinforcementCommand(TypedDict):
    """底盤(基礎底盤系)の配筋。スラブオブジェクトの ``配筋`` レコードに設定する。

    x=X 方向・y=Y 方向(鋼種を除いた呼び名文字列)。IFC の Pset_Reinforcement は
    短辺方向・長辺方向の 2 方向配筋なので、底盤の平面バウンディングボックス
    (短辺=extent の小さい軸)からグローバル X・Y 方向へ置き換えた仕様。無ければ
    既定値(D13@150 両方向)。鉄筋オブジェクトは描かず、データタグ等で参照できるよう
    スラブオブジェクトのレコードに入れる。
    """
...
WallCommand = TypedDict('WallCommand', {
    ...
    'bottom_bound': StoryBoundCommand,
    'top_bound': StoryBoundCommand,
    'reinforcement': WallReinforcementCommand,
})
...
class は割り当てる構造クラス名(立ち上がり)。reinforcement は配筋(上端主筋・
下端主筋・縦筋)で、描画フェーズが壁オブジェクトの ``配筋`` レコードに設定する。
"""
...
SlabCommand = TypedDict('SlabCommand', {
    ...
    'reinforcement': SlabReinforcementCommand,
...
reinforcement は配筋(X 方向・Y 方向)で、描画フェーズがスラブオブジェクトの
``配筋`` レコードに設定する。IFC の短辺方向・長辺方向をグローバル X・Y 方向へ
置き換えた仕様。
...
def _validate_wall(index: int, command: Any) -> None:
    ...
    for key in ('bottom_bound', 'top_bound'):
        _validate_story_bound(where, key, command.get(key))
    reinforcement = command.get('reinforcement')
    _require(isinstance(reinforcement, dict),
             f'{where}.reinforcement は dict である必要があります')
    for key in ('top', 'bottom', 'vertical'):
        _require(isinstance(reinforcement.get(key), str),
                 f'{where}.reinforcement.{key} は文字列である必要があります')
