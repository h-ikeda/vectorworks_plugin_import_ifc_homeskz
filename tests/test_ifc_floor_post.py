...
class TestShinReference:
    # 幅 105mm の土台を x=0 の位置に Y 方向へ通す(芯線 x=0、区間 y∈[-1000,1000])。
    DODAI: list[floor_post._DodaiLine] = [(0.0, -1000.0, 0.0, 1.0, 2000.0, 105.0)]

    def test_end_inset_from_dodai_returns_shin(self) -> None:
        # 大引端が土台芯より半土台厚(52.5mm)内側にある(x=52.5)→ 土台芯 x=0 を返す
        ref = floor_post._shin_reference(52.5, 300.0, 1.0, 0.0, self.DODAI)
        assert ref is not None
        assert abs(ref[0] - 0.0) < 1e-6
        assert abs(ref[1] - 300.0) < 1e-6

    def test_end_flush_with_shin_returns_same_point(self) -> None:
        # 端が土台芯ちょうど(x=0)なら同じ点を返す
        ref = floor_post._shin_reference(0.0, 100.0, 1.0, 0.0, self.DODAI)
        assert ref is not None
        assert abs(ref[0]) < 1e-6

    def test_end_far_from_dodai_returns_none(self) -> None:
        # 端が土台の footprint(半土台厚)より遠い(x=200)→ 受けていない
        assert floor_post._shin_reference(200.0, 300.0, 1.0, 0.0, self.DODAI) is None

    def test_parallel_dodai_returns_none(self) -> None:
        # 大引と平行な土台は交点が定まらない(受けとみなさない)
        assert floor_post._shin_reference(52.5, 300.0, 0.0, 1.0, self.DODAI) is None

    def test_outside_dodai_segment_returns_none(self) -> None:
        # 交点が土台の区間外(y=5000 は区間 [-1000,1000] の外)→ 受けていない
        assert floor_post._shin_reference(52.5, 5000.0, 1.0, 0.0, self.DODAI) is None
...
class TestBuildFromFixture:
    ...
    def test_posts_measured_from_dodai_shin(self) -> None:
        # 床束の総数は各大引の「土台芯どうしの区間」に _post_offsets を適用した
        # 合計と一致する(端部=実部材端ではなく土台芯。910mm 以下の区間は 0 本)。
        ...
        ifc = _open(self.FILENAME)
        dodai = floor_post._collect_dodai_lines(ifc)
        ohbiki = 0
        expected = 0
        for element in list(ifc.by_type('IfcBeam')) + list(ifc.by_type('IfcMember')):
            ...
            start = floor_post._shin_reference(ox, oy, ux, uy, dodai) or (ox, oy)
            end = floor_post._shin_reference(ex, ey, ux, uy, dodai) or (ex, ey)
